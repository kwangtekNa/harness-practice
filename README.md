...
> **하네스는 어떻게 "켜지나"?**
> - `CLAUDE.md` 가드레일은 이 폴더에서 Claude Code 를 열면 **자동 적용**됩니다(모호하면 되묻고, 금액은 Decimal 등).
> - 하지만 **오케스트레이션 워크플로는 자동으로 굴러가지 않습니다.** 세션에서 **`/harness <요청>`** 을 입력하거나 `scripts/execute.py` 를 직접 실행해 시작하세요.
>   (예: `/harness 거래내역 대시보드 만들기 — 원/막대 그래프, 세금 적용`)
> - 자연어 요청만 하면 **가드레일만** 적용되고 워크플로는 시작되지 않습니다. 이 저장소엔 자동 트리거 스킬이 없습니다(실습 단순화).
> - 처음 클론하면 Claude Code 가 **폴더 신뢰(trust)** 를 묻습니다 — 승인해야 `.claude/`(슬래시 명령·훅)가 로드되어 `/harness` 가 보입니다.
